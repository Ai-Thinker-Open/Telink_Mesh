...
void rf_link_light_event_callback (u8 status)
{
	...
	if(status == LGT_CMD_SET_MESH_INFO)
	{
		...
		cfg_led_event(LED_EVENT_FLASH_2HZ_1T);
	}
#endif
}
...
void light_sim_light2light(void){
	static u32 last_time = 0;
	if(clock_time_exceed(last_time, 1500*1000)){
        if(!is_tx_cmd_busy()){   // it must be used, if want enough bridges(default is BRIDGE_MAX_CNT 8).
    		last_time = clock_time();
    		static u8 cnt = 0;
    		u8 op_para[16] = {0};
...
void light_user_func(void){
	...
	if(deep_time == 0){
		deep_time = clock_time() | 1;
	}
	if(clock_time_exceed(deep_time, 5*1000*1000)){
		extern u32 tick_per_us;
		//cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_TIMER, clock_time() + 5*1000*1000 * tick_per_us);  //no-retention deepsleep
		cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM, PM_WAKEUP_TIMER, clock_time() + 5*1000*1000 * tick_per_us);  //retention deepsleep
	}
#endif
...
#if(WORK_SLEEP_EN)
	extern void main_loop_sleep_proc(void);
	main_loop_sleep_proc();
#endif
...
void  user_init(void)
{
    ...
    foreach_arr(i,dbg_pin){
        gpio_set_func(dbg_pin[i], AS_GPIO);
...
#if DUAL_MODE_ADAPT_EN
	dual_mode_en_init();    // must before factory_reset_handle and erase ota data, because "dual_mode_state" is used in it.
#endif
...
#if 1	
	// for app ota   
	if(0 == is_ota_area_valid(ota_program_offset)){ // 8267 / 8269 should use "ota_program_offset"
...
void retention_test()
{
    static u8 A_retention_test;
    extern int slave_link_state;
    if(A_retention_test && (slave_link_state != FLG_SYS_LINK_CONNECTED)){
        A_retention_test = 0;
        u32 t_ms = ((slave_link_state == FLG_SYS_LINK_ADV) || (slave_link_state == FLG_SYS_LINK_LISTEN)) ? 10000 : 100;
        cpu_sleep_wakeup(DEEPSLEEP_MODE_RET_SRAM, PM_WAKEUP_TIMER, clock_time() + t_ms*1000*16);
    }
}
#endif
