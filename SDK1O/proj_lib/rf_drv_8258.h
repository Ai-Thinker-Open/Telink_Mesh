...
{
    rf_set_tx_rx_off();                 // if not, sometimes can't not send adv or can't receive pkt in later state.
    rf_stop_trx();
}

#define clear_reg_dma_rf_tx_addr    do{reg_dma_rf_tx_addr = 0;}while(0)    //  8258 retention mode must, if not, hardware would use the address all the time.
