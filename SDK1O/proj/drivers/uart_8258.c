 ...
 * @return    none
 */
void uart_rtscfg(unsigned char Enable, UART_RTSModeTypeDef Mode, unsigned char Thresh, unsigned char Invert, GPIO_PinTypeDef GPIO)
{
    if (Enable)
    {
    	if(GPIO==GPIO_PA4)	//RTS==GPIO_PA4
    	{
			REG_ADDR8(0x586) &= 0xef; //disable GPIO_PA4 GPIO function
			REG_ADDR8(0x5a9) &= 0xfc; //enable GPIO_PA4 Pin as RTS Pin
			REG_ADDR8(0x5a9) |= 0x01; //enable GPIO_PA4 Pin as RTS Pin
			gpio_set_input_en(GPIO_PA4, 1);//enable input
			gpio_set_output_en(GPIO_PA4, 1);//enable output
    	}
    	else if(GPIO==GPIO_PB3)	//RTS==GPIO_PB3
    	{
			REG_ADDR8(0x58e) &= 0xf7; //disable GPIO_PB3 GPIO function
			REG_ADDR8(0x5aa) &= 0x3f; //enable GPIO_PB3 Pin as RTS Pin
			REG_ADDR8(0x5aa) |= 0x40; //enable GPIO_PB3 Pin as RTS Pin
			gpio_set_input_en(GPIO_PB3, 1);//enable input
			gpio_set_output_en(GPIO_PB3, 1);//enable output
		}
    	else if(GPIO==GPIO_PB6)	//RTS==GPIO_PB6
    	{
			REG_ADDR8(0x58e) &= 0xbf; //disable GPIO_PB6 GPIO function
			REG_ADDR8(0x5ab) &= 0xcf; //enable GPIO_PB6 Pin as RTS Pin
			REG_ADDR8(0x5ab) |= 0x20; //enable GPIO_PB6 Pin as RTS Pin
			gpio_set_input_en(GPIO_PB6, 1);//enable input
			gpio_set_output_en(GPIO_PB6, 1);//enable output
    	}
       	else	//RTS==GPIO_PC0
    	{
			REG_ADDR8(0x596) &= 0xfe; //disable GPIO_PC0 GPIO function
			REG_ADDR8(0x5ac) &= 0xfc; //enable GPIO_PC0 Pin as RTS Pin
			REG_ADDR8(0x5ac) |= 0x02; //enable GPIO_PC0 Pin as RTS Pin
			gpio_set_input_en(GPIO_PC0, 1);//enable input
 ...
 * @param[in]  GPIO   - CTS pin select,it can be GPIO_PA3/GPIO_PB2/GPIO_PC4/GPIO_PD1.
 * @return     none
 */
void uart_ctscfg(unsigned char Enable, unsigned char Select,GPIO_PinTypeDef GPIO)
{
    if (Enable)
    {
    	//Select cts pin
    	if(GPIO==GPIO_PA3)	//CTS==GPIO_PA3
    	{
    		REG_ADDR8(0x586) &= 0xf7; //disable GPIOA_GP3 GPIO function
    		REG_ADDR8(0x5a8) &= 0x3f; //enable GPIOA_GP3 Pin as CTS Pin
    		REG_ADDR8(0x5a8) |= 0x40; //enable GPIOA_GP3 Pin as CTS Pin
    		gpio_set_input_en(GPIO_PA3, 1);//enable input
    	}
    	else if(GPIO==GPIO_PB2)	//CTS==GPIO_PB2
    	{
			REG_ADDR8(0x58e) &= 0xfb; //disable GPIOD_GP1 GPIO function
			REG_ADDR8(0x5aa) &= 0xcf; //enable GPIOD_GP1 Pin as CTS Pin
			REG_ADDR8(0x5aa) |= 0x10; //enable GPIOD_GP1 Pin as CTS Pin
			gpio_set_input_en(GPIO_PB2, 1);//enable input
    	}
    	else if(GPIO==GPIO_PC4)	//CTS==GPIO_PC4
    	{
			REG_ADDR8(0x596) &= 0xef; //disable GPIOC_GP4 GPIO function
			REG_ADDR8(0x5ad) &= 0xfc; //enable GPIOC_GP4 Pin as CTS Pin
			REG_ADDR8(0x5ad) |= 0x01; //enable GPIOC_GP4 Pin as CTS Pin
			gpio_set_input_en(GPIO_PC4, 1);//enable input
//			gpio_setup_up_down_resistor(GPIO_PC4, PM_PIN_PULLDOWN_100K);
    	}
    	else	//CTS==GPIO_PD1
    	{
			REG_ADDR8(0x59e) &= 0xfd; //disable GPIOD_GP1 GPIO function
			REG_ADDR8(0x5ae) &= 0xf3; //enable GPIOD_GP1 Pin as CTS Pin
			REG_ADDR8(0x5ae) |= 0x08; //enable GPIOD_GP1 Pin as CTS Pin
			gpio_set_input_en(GPIO_PD1, 1);//enable input
			gpio_setup_up_down_resistor(GPIO_PD1, PM_PIN_PULLDOWN_100K);
    	}
...
u8 uart_IRQSourceGet(void){
	unsigned char irqS;
	irqS = reg_dma_irq_status;
	reg_dma_rx_rdy0 = irqS;

    return (irqS & (FLD_DMA_CHN_UART_RX|FLD_DMA_CHN_UART_TX));   
}
