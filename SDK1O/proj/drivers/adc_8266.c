...
{
	// pin selection
	reg_adc_chn_m_sel = adc_chn;
	adc_chn_m_input = adc_chn;
    ...
    // Set Reference voltage
  	reg_adc_ref = adc_ref;		// ref: 0 1.3v  04 AVDD
...
void adc_power_down(void){
	...
	/*step 0*/
	write_reg8(0x80002c,0x12);  //chnm: GND
	write_reg8(0x800069,0x01);  //set adc clk = 192M/(192*4) = 0.25K
	write_reg8(0x800035,0x80);  //start

	sleep_us(200);
	/*step 1*/
	write_reg8(0x800035,0x80);  //start again
	sleep_us(26);             //14 us
...
void adc_power_down_start(void){
	...
	write_reg8(0x80002c,0x12);  //chn: GND
	write_reg8(0x800069,0x01);  //set adc clk = 192M/(192*4) = 0.25K
	write_reg8(0x800035,0x80);  //start
}
...
{
    #if (ADC_CHNM_REF_SRC == ADC_REF_AVDD)
    return 3300;
    #elif(ADC_CHNM_REF_SRC == ADC_REF_1_3V)
    return 1260;
    #endif
}
...
{
    adc_Init(ADC_CHNM_ANA_INPUT, ADC_CHNM_REF_SRC);
}
#endif
