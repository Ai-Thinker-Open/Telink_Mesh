...
static inline void config_adc_channel1 (int chn_mic) {
	...
	if (chn_mic) {
		...
		analog_write (raga_pga_gain1, 0x13);
	}
	else {
...
static inline void config_adc (int chn_mic, int chn_adc, int sample) {
    ...
    reg_dfifo_scale       = 0x34; // reg0xb04[3:0] down scale by 5, reg0xb04[6:4] cic filter output select 5 :cic[22:5]
	  reg_adc_period_chn0   = 0x48; // reg0x30 T1~4,  200(16M/80K) -128 = (72 /4 ) <<2 = 72 , 0x30[1:0] just for fine tuning
    ...
    reg_aud_alc_vol       = 0x18; // 0xb06 alc volume, corespond to  reg0xb04[6:4]
	}
}
