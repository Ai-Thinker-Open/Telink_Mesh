...
#if 1
#define BP_ALWAYS(p)		if(bp_enable){bp_pos = p;bp_continue = 0;CLR_FLD(reg_tmr_ctrl,FLD_TMR0_EN);do{}while(bp_enable && 0 == bp_continue);SET_FLD(reg_tmr_ctrl,FLD_TMR0_EN);}
#define BP_ON_COUNTER(p,c)	if(bp_enable){bp_pos = p;++bp_counter;CLR_FLD(reg_tmr_ctrl,FLD_TMR0_EN);do{}while(bp_enable && (bp_counter == c));SET_FLD(reg_tmr_ctrl,FLD_TMR0_EN);}
#define BP_ON_COND(p,c)		if(bp_enable){bp_pos = p;bp_continue = 0;CLR_FLD(reg_tmr_ctrl,FLD_TMR0_EN);do{}while(bp_enable && (0 == bp_continue) && (c));SET_FLD(reg_tmr_ctrl,FLD_TMR0_EN);}
#else
