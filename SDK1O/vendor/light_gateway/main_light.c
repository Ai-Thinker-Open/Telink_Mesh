...
void  user_init(void)
{
	...
	#else
	uart_recbuff_init(hci_rx_fifo_b, hci_rx_fifo.size, (u8 *)(&T_txdata_buf));
	#endif
	...
	reg_dma_irq_status = FLD_DMA_CHN_UART_RX | FLD_DMA_CHN_UART_TX;
    ...
    #if PROVISIONING_ENABLE// callback for node status changed
