...
int mesh_config_device_group (u16 group);
void mesh_command_onoff (u16 group);
        ...
        if(vendor_id == VENDOR_ID){
            ...
            if(op == LGT_CMD_LIGHT_ONOFF){
        		...
        		if(params[0] == LIGHT_DEL_GRP_PARAM){
        			...
        			if(mesh_config_device_group(val)){
        			    cfg_led_event(LED_EVENT_FLASH_1HZ_4S);
        			}
        		}
        	    ...
        	    if(params[0] == 0xFE){
        	        ...
        	        led_lum = led_lum_tmp;
        	        music_time = 0;
        	    }else if(params[0] > 100 || params[0] < 5 || led_lum == params[0]){
                    return;
                }else{
                    led_lum = params[0];
                }
...
////////////////////////////////////////////////////////////////////////////
extern rf_packet_att_cmd_t	pkt_user_cmd;
u8			mesh_cmd_buff[256];
u16			cmd_dst = 0xffff;
u8			cmd_op_para[16];
u32			cmd_sno;
u8			cmd_busy = 0;

void mesh_command(u8 cmd)
{
	u16 group = group_address[0];
	if (!group)
	{
		group = 0xffff;
	}
    ...
    if (1)
	{
		...
		cmd_op_para[3] = cmd == LGT_CMD_LIGHT_ON;
	}
...
void proc_ui (void)
{
	...
	u8	btn = gpio_read (GPIO_PD4);
	if (!btn && button0)
	{
		mesh_command (LGT_CMD_LIGHT_ON);
	}
	button0 = btn;

	btn = gpio_read (GPIO_PD5);
	if (!btn && button1)
	{
		mesh_command_onoff (0xffff);
		mesh_command (LGT_CMD_LIGHT_OFF);
	}
	button1 = btn;
}

#define IO_SWD                 0x8000b0
#define IO_SWC                 0x8000b1
#define WAIT_SWIRE_DONE while (read_reg8(0x8000b1) & 3)

void SwireWrite (unsigned short adr, unsigned char *p, int n)
{
	...
	write_reg8 (IO_SWD, adr>>8);
	write_reg8 (IO_SWC, 0x01);
	WAIT_SWIRE_DONE;

	write_reg8 (IO_SWD, adr);
	write_reg8 (IO_SWC, 0x01);
	WAIT_SWIRE_DONE;
...
void mesh_command_onoff(u16 group)
{
	...
	cmd_op_para[1] = VENDOR_ID & 0xFF;
	cmd_op_para[2] = VENDOR_ID >> 8;
	cmd_op_para[3+3] = ON_OFF_FROM_PASSIVE_DEV;

	cmd_sno += clock_time ()<<8;
	
	u16 adr_src_backup = device_address;
	device_address = 0;                 // use 0 address for passive device
	mesh_push_user_command_no_relay((int)(cmd_sno++), cmd_dst, cmd_op_para, 11);
	
    if (security_enable)
    {
    	pkt_user_cmd.type |= BIT(7);
    	pair_enc_packet_mesh ((u8 *)&pkt_user_cmd);
    }
	memcpy (mesh_cmd_buff + 8, &pkt_user_cmd.type, 48);
	mesh_packet_tx_crc (mesh_cmd_buff + 8, mesh_cmd_buff + 8);

	cmd_sno ^= (clock_time ()<<16);
	cmd_op_para[3] = 0;
	mesh_push_user_command_no_relay((int)(cmd_sno++), cmd_dst, cmd_op_para, 11);
	...
	memcpy (mesh_cmd_buff + 56, &pkt_user_cmd.type, 48);
	mesh_packet_tx_crc (mesh_cmd_buff + 56, mesh_cmd_buff + 56);

	memcpy (mesh_cmd_buff, sys_chn_listen, 4);
	memcpy (mesh_cmd_buff + 4, (u8 *)&pair_ac, 4);
	mesh_cmd_buff[255] = 0xa5;
	//while (1);
	//u8 r = irq_disable ();
	SwireWrite (0x8000, mesh_cmd_buff, 256);
	//irq_restore (r);
	device_address = adr_src_backup;
}


int mesh_config_device_group (u16 group)
{
	//////// generate on/off command & write to device under configure through SWB
	return 1;
}
////////////////////////////////////////////////////////////////////////////
void main_loop(void)
{
	...
	//flash_protect_debug();
#if SW_GET_MESH_DATA
	static u32 ClockCont = 0;
	if(SWClock && clock_time_exceed(SWClock, 1000*1000)){
		if(SWTxCnt && (!clock_time_exceed(ClockCont, 100*1000)))
			;
		else{
			light_respond_to_sw();
			ClockCont = clock_time();
			if(++SWTxCnt == 5){
				SWClock = 0;
				SWTxCnt = 0;
			}
...
void  user_init(void)
{
	...
	if(p_vendor_user_init){
		...
		pwm_set (PWMID_B, PMW_MAX_TICK, 0);
