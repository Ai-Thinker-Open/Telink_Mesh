...
#define UART_DATA_LEN    76      // data max 252
typedef struct{
...
#if UART_FIFO_ENABLE
MYFIFO_INIT(hci_rx_fifo, (UART_DATA_LEN + 8), 4);
MYFIFO_INIT(hci_tx_fifo, (UART_DATA_LEN + 8), 4);
#endif
#endif
...
_attribute_ram_code_ void irq_handler(void)
{
    ...
    if(irqS & UARTRXIRQ_MASK){
        enterRXIrq++;
        //reg_dma_rx_rdy0 = FLD_DMA_UART_RX; Has done in uart_IRQSourceGet
    ...
    if(irqS & UARTTXIRQ_MASK){
		...
		//reg_dma_rx_rdy0 = FLD_DMA_UART_TX;Has done in uart_IRQSourceGet
        enterTXIrq++;
    }
#else
    if(irqS & UARTRXIRQ_MASK){
...
int hci_tx_fifo_poll()
{
    
    u8 *p_fifo = my_fifo_get (&hci_tx_fifo);
    #if(MCU_CORE_TYPE == MCU_CORE_8267)
    unsigned char uart_tx_is_busy();
	if(p_fifo && !uart_tx_is_busy ())
    #else
    if(p_fifo)
    #endif
	{
...
void main_loop(void)
{
	...
	proc_ui();
    if(mode_master){
		master_dongle_main_loop();
        #if UART_FIFO_ENABLE
        static u32 tx_ref;
        uart_ErrorCLR();
        if(clock_time_exceed(tx_ref, 30000))
        {
            unsigned char uart_tx_is_busy();
            if(hci_tx_fifo_poll())
            {
...
void uart_init()
{
    ...
    CLK32M_UART115200;
#if UART_FIFO_ENABLE
    uart_BuffInit(hci_rx_fifo_b, hci_rx_fifo.size, (u8 *)(&T_txdata_buf));
#else
...
#if MODE_MASTER_SLAVE
    master_dongle_button_init();
	mode_master = master_dongle_get_mode();
    if(mode_master){
		master_dongle_user_init();
        #if UART_ENABLE
        uart_init();    //Must after master_dongle_user_init
        #endif
    	return;
    }
#endif    
